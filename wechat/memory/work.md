...
## 📌 快速开始（每次开工先看这里，最后更新 2026-08-09 v2.5.0）

**当前状态**
- 网页（云托管）：v2.5.0 已上线，新增**网页版微信扫码登录（小程序码）**。登录按钮改为「💬 微信扫码登录」→ 弹窗显示小程序码 → 微信扫码 → 小程序确认 → 网页轮询拿到 token 自动登录
- 小程序：**v2.5.0 已上传开发版（含 pages/qr-login 确认页），待发布**。v2.4.0~v2.4.10 亦均已上传但从未发布，线上仍是旧版
- **待办（人工，无法脚本化，影响扫码登录是否可用）**：
  1. **小程序 API IP 白名单**：云托管容器出口 IP `49.234.141.38` 需加入小程序后台「开发管理 → 开发设置 → IP 白名单」，否则生成小程序码报 `invalid ip ... not in whitelist`（已验证）。`WECHAT_APP_SECRET` 云端已配置（调用已到达微信）
  2. **小程序发布**：v2.5.0 开发版需提交审核并发布，否则 getwxacodeunlimit 生成的码无法打开 qr-login 页（扫码登录仅对线上版生效）
